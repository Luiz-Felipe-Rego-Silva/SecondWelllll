...
using System.Linq;
using Model.Detailing.Entities.Cut;
...
namespace Controllers
{
    class CircularBottomDetailing
    {
        ...
        public void SetBorderShape(bool[] conditions)
        {
            ...
            {
                ExtraLength = StandardDistribuction.GetAnchorLength(GaugeY / 10.0, AnchorFactor) + 15.0,
                ExtesionThickness = Bottom.Thickness
            };
            VerticalCut.AddBorder(conditions);

            HorizontalCut = new HorizontalCut(SupWallThickness, InfWallThickness, Bottom.Diameter)
            {
                ExtraLength = StandardDistribuction.GetAnchorLength(GaugeX / 10.0, AnchorFactor) + 15.0,
                ExtesionThickness = Bottom.Thickness
            };
        ...
        public void DrawDetailment(Point3d BasePoint)
        {
            ...
            Point3d bottomCenter = new Point3d(BasePoint.X + 2 * StandardDistribuction.GetAnchorLength(GaugeX / 10.0, AnchorFactor) + 0.5 * Bottom.Diameter + 100.0, BasePoint.Y - 2 * StandardDistribuction.GetAnchorLength(GaugeY / 10.0, AnchorFactor) - 200.0 - 0.5 * Bottom.Diameter, 0);
        ...
        private void DrawVerticalDistribuctions(Point3d startPointBars)
        {
            ...
            Point3d negativeStartPoint = new Point3d(startPointBars.X + 2 * StandardDistribuction.GetAnchorLength(GaugeY / 10.0, AnchorFactor) + 50.0, startPointBars.Y + negativeVerticalBar.MediumLength / 2.0, startPointBars.Z);
            Point3d positiveStartPoint = new Point3d(startPointBars.X, startPointBars.Y + positiveVerticalBar.MediumLength / 2.0, startPointBars.Z);
